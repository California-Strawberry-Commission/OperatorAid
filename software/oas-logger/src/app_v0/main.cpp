...
/**
 * Monitors for deep sleep conditions. When conditions are met, transitions to
 * OFFLOAD then SLEEP state.
 *
 * Continuously checks for deep sleep conditions. After the sleep button is
 * pressed, if released before the long press duration, consider that a short
 * press. Otherwise, if the sleep button is held down for longer than the long
 * press threshold, consider that a long press.
 * - Short press: go to offload then sleep
 * - Long press: reset WiFi credentials and reboot
 * In addition, if external power is disconnected for more than 2 seconds, go
 * to offload then sleep.
 */
void sleepMonitorTask(void* args) {
